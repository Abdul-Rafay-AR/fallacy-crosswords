...
function crosswordPuzzle(puzzlewords) {
    ...
    if (!graphs || !graphs.length) {
        console.log(
            "Developer Error : Your words could not be made into graphs."
        );
        return false;
    }
...
function showCrossWordOptions() {
    ...
    var solvefunction = function () {
        ...
        var acrosstext =
            $(this).attr("data-across") == "false" ? "Down" : "Across";
        $("#position-and-clue").html(
            "<b>" + acrosstext + "</b> : " + $(this).attr("data-clue")
        );
        $("#answer-form").show();
...
function showCrossWordLists(wordlists, clues) {
    ...
    var downlistordered = fillInCrossWordNumbers(
        downlist,
        acrosslist,
        acrosslistordered
    );

    var acrosslistorderedelement = getViewableCrossWordList(
        acrosslistordered,
        clues,
        true
    );
    var downlistorderedelement = getViewableCrossWordList(
        downlistordered,
        clues,
        false
    );
...
function fillInCrossWordNumbers(listitems, blockitems, blockitemsordered) {
    ...
    for (var i = 0; i < listitems.length; i++) {
        ...
        var blockingitemnumber = getBlockingItemNumber(
            coordinates,
            blockitems,
            blockitemsordered
        );
        ...
        var element =
            '<div class="background-text"><span class="crossword-grid-cell-number">' +
            fillnumber +
            "</span></div>";
        ...
        parentelement = $(
            "#cell-position-" + coordinates[0] + "-" + coordinates[1]
        );
...
function getBlockingItemNumber(coordinates, blockitems, blockitemsordered) {
    ...
    for (var i = 0; i < blockitems.length; i++) {
        ...
        if (
            blockcoordinates[0] == coordinates[0] &&
            blockcoordinates[1] == coordinates[1]
        ) {
            return getBlockItemNumberPosition(
                blockitem["word"],
                blockitemsordered
            );
        }
...
function showCrossWordPuzzle(matrix) {
    ...
    var table = $(
        '<table class="puzzle" border="1" cellpadding="0" cellspacing="0"></table>'
    );
    ...
    for (var i = 0; i < tallestline; i++) {
        ...
        for (var j = 0; j < widestline; j++) {
            ...
            tablerow +=
                '<span class="letter-text" id="letter-position-' +
                i +
                "-" +
                j +
                '">';
...
function buildCrosswordBlockGraphs(graphs) {
    ...
    for (var i = 0; i < graphs.length; i++) {
        ...
        var buildvertically = checkToBuildVertically(
            fullmatrix,
            matrix,
            widestline,
            tallestline
        ...
        if (!buildvertically) {
            ...
            for (var j = 0; j < fullmatrix.length; j++) {
                ...
                if (
                    trimmedfullmatrixline.length > 0 &&
                    trimmedfullmatrixline.length < shortestlinelength
                ) {
                    var solutioncoordinates = [
                        trimmedfullmatrixline.length,
                        j + i,
                    ];
                    var newerpossiblefullmatrixsolution =
                        joinHorizontalMatrices(
                            fullmatrix,
                            matrix,
                            solutioncoordinates
                        );
                    if (newerpossiblefullmatrixsolution) {
                        shortestlinelength = getThinnestLine(
                            newerpossiblefullmatrixsolution
                        );
                        possiblefullmatrixsolution =
                            newerpossiblefullmatrixsolution;
                        ...
                        while (
                            canmutate &&
                            trimmedfullmatrixline.length - leftpushback >= 0
                        ) {
                            ...
                            if (newestpossiblefullmatrixsolution) {
                                shortestlinelength = getThinnestLine(
                                    newestpossiblefullmatrixsolution
                                );
                                possiblefullmatrixsolution =
                                    newestpossiblefullmatrixsolution;
                                possiblefullmatrixcoordinates =
                                    solutioncoordinates;
                                leftpushback++;
                            } else {
                                canmutate = false;
            ...
            if (possiblefullmatrixsolution) {
                ...
                console.info(matrixpositions);
                matrixpositions = interpolateMatrixPositions(matrixpositions, [
        ...
        if (buildvertically || !built) {
            ...
            for (var j = 0; j < widestline; j++) {
                ...
                if (nonConflictingRows(fullmatrixbottom, smallmatrixtop)) {
                    ...
                    matrixpositions = interpolateMatrixPositions(
                        matrixpositions,
                        solutioncoordinates
                    );
                    fullmatrixpositions.push({
            ...
            if (!built) {
                ...
                matrix = compactCrosswordBlockSource({ matrix: matrix })[
                    "matrix"
                ];
                fullmatrix.push("");
                fullmatrix = joinVerticalMatrices(fullmatrix, matrix);
        ...
        fullmatrix = compactCrosswordBlockSource({ matrix: fullmatrix })[
            "matrix"
        ];
    }
...
function joinHorizontalMatrices(fullmatrix, matrix, coordinates) {
    ...
    for (var i = 0; i < matrix.length; i++) {
        ...
        for (var j = 0; j < line.length; j++) {
            ...
            if (matrix[i][j] != " ") {
                fullmatrix[y] = insertLetterAtStringPosition(
                    matrix[i][j],
                    fullmatrix[y],
                    x
                );
...
function buildCrosswordBlocks(crosswordblocks) {
    ...
    for (var word in crosswordblocks) {
        ...
        if (across) {
            ...
            for (var i = 0; i < subwords.length; i++) {
                ...
                var matchingposition = findMatchingLetterMatrixPosition(
                    matrix,
                    word,
                    subletter,
                    longestwordlength - 2
            ...
            matrix = fillLetterMatrixVertically(
                matrix,
                word,
                longestwordlength + 1,
                0
            ...
            for (var i = 0; i < subwords.length; i++) {
                ...
                var subword = subwordentry[0];
                var subletter = subwordentry[1];
                var matchingposition = findMatchingLetterMatrixPositionVertical(
                    matrix,
                    word,
                    subletter,
                    longestwordlength - 1
    ...
    if (crosswordblocks["(unmatched)"]) {
        var graph = buildUnassignedCrosswordBlock(
            crosswordblocks["(unmatched)"]
        );
        graphs.push(graph);
    }
...
function findMatchingLetterMatrixPositionVertical(
    matrix,
    word,
    subletter,
    index
...
function compactCrosswordBlockTop(graph) {
    ...
    for (var i = 0; i < crosswordblocklength; i++) {
        ...
        if (!row || !trimmedrow.length) {
            ...
            crosswordblocksolutions = incrementCrossWordBlockHeights(
...
function compactCrosswordBlockLeft(graph) {
    ...
    while (shorten) {
        ...
        if (shorten) {
            ...
            crosswordblocksolutions = incrementCrossWordBlockLengths(
                crosswordblocksolutions
            );
        }
    }
...
function compactCrosswordBlockRight(graph) {
    ...
    while (shorten) {
        ...
        if (shorten) {
            ...
            for (var i = 0; i < crosswordblocklength; i++) {
                var crosswordrow = crosswordblock[i];
                crosswordblock[i] = crosswordrow.substr(
                    0,
        ...
        if (!checkedcrosswords[checkedcrosswordkey]) {
            ...
            for (var j = i + 1; j < shuffledwords.length; j++) {
                ...
                if (!checkedcrosswords[nextcrosswordkey]) {
                    var matchingletter = getMatchingLetter(
                        wordletters,
                        nextword
                    );
